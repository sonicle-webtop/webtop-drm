...
import com.sonicle.webtop.drm.swagger.v2.api.CompaniesApi;
 ...
 */
public class Companies extends CompaniesApi {
	
	private static final Logger logger = LoggerFactory.getLogger(Companies.class);

	@Override
	public Response listCompanies(String userId) {
		...
		try {
			List<OCompany> list = manager.listCompaniesByDomainUser(); 
			ApiCompaniesResult acr = new ApiCompaniesResult();
			ArrayList<ApiCompany> items = new ArrayList<>();
			for (OCompany oc: list) {
				ApiCompany c = new ApiCompany();
				c.setId(oc.getCompanyId().toString());
				c.setDisplayname(oc.getName());
				items.add(c);
			}
			acr.setItems(items);
			return respOk(acr);
		} catch(WTException exc) {
			logger.error("[{}] getCompanies({})", currentProfileId, userId, exc);
