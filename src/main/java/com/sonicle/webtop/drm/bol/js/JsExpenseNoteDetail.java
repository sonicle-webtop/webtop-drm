 ...
 * @author lssndrvs
 */
public class JsExpenseNoteDetail {
	...
	public List<Document> detailDocuments = new ArrayList();
	
	public JsExpenseNoteDetail(ExpenseNoteDetail eND) {
		this.id = eND.getId();
		...
		detailDocuments = new ArrayList<>(eND.getDetailDocuments().size());
		for (ExpenseNoteDetailDocument doc : eND.getDetailDocuments()) {
			Document jsdoc = new Document();
			jsdoc.id = doc.getId();
			jsdoc.name = doc.getFileName();
			jsdoc.size = doc.getSize();
			detailDocuments.add(jsdoc);
		}
	}
	
	public static ExpenseNoteDetail createExpenseNoteDetail(JsExpenseNoteDetail js) {
